...
export const PINTEREST_OAUTH_LOCALE_COOKIE = "pinterest_oauth_locale";
export const PINTEREST_OAUTH_COOKIE_MAX_AGE = 60 * 10;
...
export function redirectToAccountsAfterPinterest(
  request: NextRequest,
  locale: Locale,
...
export function setPinterestOAuthCookies(
  ...
  response.cookies.set(PINTEREST_OAUTH_STATE_COOKIE, state, {
    ...
    sameSite: "lax",
    maxAge: PINTEREST_OAUTH_COOKIE_MAX_AGE,
    path: "/",
  });

  response.cookies.set(PINTEREST_OAUTH_LOCALE_COOKIE, locale, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax",
    maxAge: PINTEREST_OAUTH_COOKIE_MAX_AGE,
