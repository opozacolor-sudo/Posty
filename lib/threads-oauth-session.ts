...
export const THREADS_OAUTH_LOCALE_COOKIE = "threads_oauth_locale";
export const THREADS_OAUTH_COOKIE_MAX_AGE = 60 * 10;
...
export function redirectToAccountsAfterThreads(
  request: NextRequest,
  locale: Locale,
...
export function setThreadsOAuthCookies(
  ...
  response.cookies.set(THREADS_OAUTH_STATE_COOKIE, state, {
    ...
    sameSite: "lax",
    maxAge: THREADS_OAUTH_COOKIE_MAX_AGE,
    path: "/",
  });

  response.cookies.set(THREADS_OAUTH_LOCALE_COOKIE, locale, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax",
    maxAge: THREADS_OAUTH_COOKIE_MAX_AGE,
