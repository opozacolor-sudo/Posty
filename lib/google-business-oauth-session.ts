...
export const GOOGLE_BUSINESS_OAUTH_LOCALE_COOKIE = "google_business_oauth_locale";
export const GOOGLE_BUSINESS_OAUTH_COOKIE_MAX_AGE = 60 * 10;
...
export function redirectToAccountsAfterGoogleBusiness(
  request: NextRequest,
  locale: Locale,
...
export function setGoogleBusinessOAuthCookies(
  ...
  response.cookies.set(GOOGLE_BUSINESS_OAUTH_STATE_COOKIE, state, {
    ...
    sameSite: "lax",
    maxAge: GOOGLE_BUSINESS_OAUTH_COOKIE_MAX_AGE,
    path: "/",
  });

  response.cookies.set(GOOGLE_BUSINESS_OAUTH_LOCALE_COOKIE, locale, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    sameSite: "lax",
    maxAge: GOOGLE_BUSINESS_OAUTH_COOKIE_MAX_AGE,
